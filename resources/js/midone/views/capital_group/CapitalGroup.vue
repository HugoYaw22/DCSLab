<template>
    ...
    <div class="intro-y" v-if="mode === 'list'">
        <DataList :title="t('views.capital_group.table.title')" :data="capital_groupList" v-on:createNew="createNew" v-on:dataListChange="onDataListChange" :enableSearch="true">
           <template v-slot:table="tableProps">
                <table class="table table-report -mt-2">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ t('views.capital_group.table.cols.code') }}</th>
                            <th class="whitespace-nowrap">{{ t('views.capital_group.table.cols.name') }}</th>
                            ...
                            <tr :class="{'intro-x':true, 'hidden transition-all': expandDetail !== itemIdx}">
                                <td colspan="6">
                                    <div class="flex flex-row">
                                        <div class="ml-5 w-48 text-right pr-5">{{ t('views.capital_group.fields.code') }}</div>
                                        <div class="flex-1">{{ item.code }}</div>
                                    </div>
                                    <div class="flex flex-row">
                                        <div class="ml-5 w-48 text-right pr-5">{{ t('views.capital_group.fields.name') }}</div>
    ...
    <div class="intro-y box" v-if="mode !== 'list'">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
            <h2 class="font-medium text-base mr-auto" v-if="mode === 'create'">{{ t('views.capital_group.actions.create') }}</h2>
            <h2 class="font-medium text-base mr-auto" v-if="mode === 'edit'">{{ t('views.capital_group.actions.edit') }}</h2>
        </div>
        <div class="loader-container">
            <VeeForm id="capital_groupForm" class="p-5" @submit="onSubmit" @invalid-submit="invalidSubmit" v-slot="{ handleReset, errors }">
                <div class="p-5">                    
                    <!-- #region Code -->
                    <div class="mb-3">
                        <label for="inputCode" class="form-label">{{ t('views.capital_group.fields.code') }}</label>
                        <div class="flex items-center">
                            <VeeField id="inputCode" name="code" type="text" :class="{'form-control':true, 'border-danger': errors['code']}" :placeholder="t('views.capital_group.fields.code')" :label="t('views.capital_group.fields.code')" rules="required" @blur="reValidate(errors)" v-model="capital_group.code" :readonly="capital_group.code === '[AUTO]'" />
                    ...
                    <!-- #region Name -->
                    <div class="mb-3">
                        <label for="inputName" class="form-label">{{ t('views.capital_group.fields.name') }}</label>
                        <VeeField id="inputName" name="name" type="text" :class="{'form-control':true, 'border-danger': errors['name']}" :placeholder="t('views.capital_group.fields.name')" :label="t('views.capital_group.fields.name')" rules="required" @blur="reValidate(errors)" v-model="capital_group.name" />
...
//#region Data - Views
const capital_groupList = ref({});
const capital_group = ref({
    code: '',
    name: '',
});
...
const getAllCapitalGroups = (args) => {
    capital_groupList.value = {};
    ...
    axios.get(route('api.get.db.cash.capital_group.read', { "companyId": companyId, "page": args.page, "perPage": args.pageSize, "search": args.search })).then(response => {
        capital_groupList.value = response.data;
        loading.value = false;
    });
}
...
const onSubmit = (values, actions) => {
    ...
    var formData = new FormData(dom('#capital_groupForm')[0]); 
    formData.append('company_id', selectedUserCompany.value);
    
    if (mode.value === 'create') {
        axios.post(route('api.post.db.cash.capital_group.save'), formData).then(response => {
            backToList();
        }).catch(e => {
        ...
        axios.post(route('api.post.db.cash.capital_group.edit', capital_group.value.hId), formData).then(response => {
            actions.resetForm();
            backToList();
        }).catch(e => {
            handleError(e, actions);
...
const createNew = () => {
    ...
    if (sessionStorage.getItem('DCSLAB_LAST_ENTITY') !== null) {
        capital_group.value = JSON.parse(sessionStorage.getItem('DCSLAB_LAST_ENTITY'));
        sessionStorage.removeItem('DCSLAB_LAST_ENTITY');
    } else {
        capital_group.value = emptyCapitalGroup();
    }
    capital_group.value.company.hId = _.find(companyDDL.value, { 'hId': selectedUserCompany.value });
}

const onDataListChange = ({page, pageSize, search}) => {
    getAllCapitalGroups({page, pageSize, search});
}

const editSelected = (index) => {
    mode.value = 'edit';
    capital_group.value = capital_groupList.value.data[index];
}

const deleteSelected = (index) => {
    deleteId.value = capital_groupList.value.data[index].hId;
    deleteModalShow.value = true;
}

const confirmDelete = () => {
    deleteModalShow.value = false;
    axios.post(route('api.post.db.cash.capital_group.delete', deleteId.value)).then(response => {
        backToList();
    }).catch(e => {
...
const backToList = () => {
    ...
    mode.value = 'list';
    getAllCapitalGroups({ page: capital_groupList.value.current_page, pageSize: capital_groupList.value.per_page });
}
...
const generateCode = () => {
    if (capital_group.value.code === '[AUTO]') capital_group.value.code = '';
    else  capital_group.value.code = '[AUTO]'
}
//#endregion
