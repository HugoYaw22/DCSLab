<template>
    ...
    <div class="intro-y" v-if="mode === 'list'">
        <DataList :title="t('views.expense_group.table.title')" :data="expense_groupList" v-on:createNew="createNew" v-on:dataListChange="onDataListChange" :enableSearch="true">
           <template v-slot:table="tableProps">
                <table class="table table-report -mt-2">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ t('views.expense_group.table.cols.code') }}</th>
                            <th class="whitespace-nowrap">{{ t('views.expense_group.table.cols.name') }}</th>
                            ...
                            <tr :class="{'intro-x':true, 'hidden transition-all': expandDetail !== itemIdx}">
                                <td colspan="6">
                                    <div class="flex flex-row">
                                        <div class="ml-5 w-48 text-right pr-5">{{ t('views.expense_group.fields.code') }}</div>
                                        <div class="flex-1">{{ item.code }}</div>
                                    </div>
                                    <div class="flex flex-row">
                                        <div class="ml-5 w-48 text-right pr-5">{{ t('views.expense_group.fields.name') }}</div>
    ...
    <div class="intro-y box" v-if="mode !== 'list'">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
            <h2 class="font-medium text-base mr-auto" v-if="mode === 'create'">{{ t('views.expense_group.actions.create') }}</h2>
            <h2 class="font-medium text-base mr-auto" v-if="mode === 'edit'">{{ t('views.expense_group.actions.edit') }}</h2>
        </div>
        <div class="loader-container">
            <VeeForm id="expense_groupForm" class="p-5" @submit="onSubmit" @invalid-submit="invalidSubmit" v-slot="{ handleReset, errors }">
                <div class="p-5">                    
                    <!-- #region Code -->
                    <div class="mb-3">
                        <label for="inputCode" class="form-label">{{ t('views.expense_group.fields.code') }}</label>
                        <div class="flex items-center">
                            <VeeField id="inputCode" name="code" type="text" :class="{'form-control':true, 'border-danger': errors['code']}" :placeholder="t('views.expense_group.fields.code')" :label="t('views.expense_group.fields.code')" rules="required" @blur="reValidate(errors)" v-model="expense_group.code" :readonly="expense_group.code === '[AUTO]'" />
                    ...
                    <!-- #region Name -->
                    <div class="mb-3">
                        <label for="inputName" class="form-label">{{ t('views.expense_group.fields.name') }}</label>
                        <VeeField id="inputName" name="name" type="text" :class="{'form-control':true, 'border-danger': errors['name']}" :placeholder="t('views.expense_group.fields.name')" :label="t('views.expense_group.fields.name')" rules="required" @blur="reValidate(errors)" v-model="expense_group.name" />
...
//#region Data - Views
const expense_groupList = ref({});
const expense_group = ref({
    code: '',
    name: '',
});
const statusDDL = ref([]);
...
const getAllExpenseGroups = (args) => {
    expense_groupList.value = {};
    if (args.pageSize === undefined) args.pageSize = 10;
    if (args.search === undefined) args.search = '';

    let companyId = selectedUserCompany.value;

    axios.get(route('api.get.db.cash.expense_group.read', { "companyId": companyId, "page": args.page, "perPage": args.pageSize, "search": args.search })).then(response => {
        expense_groupList.value = response.data;
        loading.value = false;
    });
}
...
const onSubmit = (values, actions) => {
    ...
    var formData = new FormData(dom('#expense_groupForm')[0]); 
    formData.append('company_id', selectedUserCompany.value);
    
    if (mode.value === 'create') {
        axios.post(route('api.post.db.cash.expense_group.save'), formData).then(response => {
            backToList();
        }).catch(e => {
        ...
        axios.post(route('api.post.db.cash.expense_group.edit', expense_group.value.hId), formData).then(response => {
            actions.resetForm();
            backToList();
        }).catch(e => {
            handleError(e, actions);
...
const createNew = () => {
    ...
    if (sessionStorage.getItem('DCSLAB_LAST_ENTITY') !== null) {
        expense_group.value = JSON.parse(sessionStorage.getItem('DCSLAB_LAST_ENTITY'));
        sessionStorage.removeItem('DCSLAB_LAST_ENTITY');
    } else {
        expense_group.value = emptyExpenseGroup();
    }
    expense_group.value.company.hId = _.find(companyDDL.value, { 'hId': selectedUserCompany.value });
}

const onDataListChange = ({page, pageSize, search}) => {
    getAllExpenseGroups({page, pageSize, search});
}

const editSelected = (index) => {
    mode.value = 'edit';
    expense_group.value = expense_groupList.value.data[index];
}

const deleteSelected = (index) => {
    deleteId.value = expense_groupList.value.data[index].hId;
    deleteModalShow.value = true;
}

const confirmDelete = () => {
    deleteModalShow.value = false;
    axios.post(route('api.post.db.cash.expense_group.delete', deleteId.value)).then(response => {
        backToList();
    }).catch(e => {
...
const backToList = () => {
    ...
    mode.value = 'list';
    getAllExpenseGroups({ page: expense_groupList.value.current_page, pageSize: expense_groupList.value.per_page });
}
...
const generateCode = () => {
    if (expense_group.value.code === '[AUTO]') expense_group.value.code = '';
    else  expense_group.value.code = '[AUTO]'
}
//#endregion
